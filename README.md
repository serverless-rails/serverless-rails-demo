...
aws ssm put-parameter --region $(terraform output -json | jq -r .aws_region.value) --profile serverless-rails-demo --type SecureString --name "/production/DATABASE_URL" --value "postgres://app:$(terraform output -json | jq -r .database_password.value)@$(terraform output -json | jq -r .database_host.value):5432/app"
...
aws ssm put-parameter --region $(terraform output -json | jq -r .aws_region.value) --profile serverless-rails-demo --type SecureString --name "/production/TELEGRAM_BOT_TOKEN" --value "THE-TELEGRAM-BOT-TOKEN"
