...
RUN DATABASE_URL=postgres://localhost/dummy SECRET_KEY_BASE=x bundle exec rake assets:precompile \
  && aws s3 cp /sr/public/assets s3://sr-$RAILS_ENV-client-assets/assets/ --recursive --quiet \
  && aws s3 cp /sr/public/packs s3://sr-$RAILS_ENV-client-assets/packs/ --recursive --quiet
